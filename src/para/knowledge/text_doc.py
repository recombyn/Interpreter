...
Any .text/.txt/.md under knowledge/user/ becomes line-addressable:
  of(content, line-entity, …) in .tm (hot path)
  plus knowledge/user/.content shards (D67 scale path)

No format-specific parsers. Line-entity surface format is user-language
(e.g. Chinese 「第N行」) and belongs with the Chinese surface stack.
"""
...
from para.kernel.machine import boot, save_world
...
# Docs that must not be imported as D66 world memory
_SKIP_TEXT_NAMES = frozenset(
    {
        "readme.md",
        "readme.txt",
        "readme.markdown",
...
# Surface tokens for Chinese line addressing (user language; not algorithm logic)
_EMPTY_LINE_BODY = "（空行）"
_LINE_ENTITY = "第{n}行"
_DOC_LINE_ENTITY = "{stem}第{n}行"
...
def is_user_text(path: Path) -> bool:
    if not path.is_file() or path.suffix.lower() not in TEXT_SUFFIXES:
        return False
    if path.name.casefold() in _SKIP_TEXT_NAMES:
        return False
    return True


def list_user_texts(user_dir: Path | None = None) -> list[Path]:
    """List user docs under knowledge/user/ (includes domain subfolders)."""
...
def _content_body(text: str) -> str:
    ...
    text = (text or "").strip() or _EMPTY_LINE_BODY
    text = text.lstrip("#").strip() or _EMPTY_LINE_BODY
    text = "".join(ch for ch in text if not ch.isspace())
    text = text.replace("#", "＃").replace(":", "：").replace("*", "")
    return text or _EMPTY_LINE_BODY
...
def render_memory_tm(
    ...
    *,
    ...
    for line in doc.lines:
        ...
        e1 = _LINE_ENTITY.format(n=line.no)
        e2 = _DOC_LINE_ENTITY.format(stem=stem, n=line.no)
...
def list_user_memory_tms(user_dir: Path | None = None) -> list[Path]:
    ...
    for path in sorted(root.rglob("*.tm")):
        if under_skipped_user_dir(path, root):
            continue
        if path.name.casefold() in {n.casefold() for n in _SKIP_TM}:
            continue
        try:
