...
_SHORT_ASK = re.compile(
    r"^(那呢|呢|呢？|那\s*[？?]?|然后呢|还有呢|这个呢|那个呢|"
    r"违法吗|合法吗|对吗|行吗|是吗|呢吗|"
    r"什么|为什么|怎么|怎样|如何)\s*[？?]?\s*$"
)
...
class Session:
    ...
    def expand_short_ask(self, text: str, domain: set[str] | dict | None = None) -> str | None:
        ...
        if "的内容是什么" in text:
            return None
        ent = self.pinned_content_entity(domain)
        if not ent:
            return None
        # MEM4 → D69 when pin is a judge topic and ask is legality-ish
        t = text.strip()
        if re.fullmatch(r"(违法吗|合法吗|合规吗|可以吗|对吗|行吗)\s*[？?]?", t):
            try:
                from cni.judge import judge_topics
    ...
    def prefer_doc(self) -> str:
        return self.doc_focus or ""
...
def __getattr__(name: str):
    if name == "EVENT_DEIXIS_CLASS":
        return load_system().event_deixis
